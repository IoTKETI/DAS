...
function convert_onem2m_token_to_JWT(onem2m_claimsets){
	...
	if(key == 'tkvr' || key == 'tknm' || key == 'tkps' || key == 'tkex'){
...
//function JWE_header(algorithm = 'RSA-OAEP-256', encrypt = 'A128GCM'){
function JWE_header(algorithm = 'A256KW', encrypt = 'A256GCM'){
    header = {
	"alg": algorithm,
	"enc": encrypt,
	"typ": "JWT"
...
//    key = jose.JWK.generateSync(key_type,key_size,{ use: usage },is_private) ;
    //    if(key.private){
    gen_key = jose.JWK.generateSync(key_type,key_size,{ use: usage }) ;
